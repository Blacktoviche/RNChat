...
import { Container, Content, Header, List, ListItem, Item, Input, Thumbnail, Body, Left, Right, Icon, Button, Text } from 'native-base';
        ...
        return (
            <Container>
                <Header searchBar rounded>
                    <Item>
                        <Icon name="search" />
                        <Input placeholder="Search" value={this.props.searchName}
                            onChangeText={this.onSearchContact.bind(this)} />
                        <Icon active name="people" />
                    </Item>
                </Header>
                <Content>
                    <List dataArray={this.props.filteredContactList} renderRow={(user) =>
                        <ListItem thumbnail>
                        <Left>
                            <Thumbnail square size={80} source={require('../imgz/noimg.png')} />
                        </Left>
                        <Body>
                        ...
                        <Right>
                            <Button transparent  onPress={() => this.onOpenConversation(user)}>
                                <Text>Chat</Text>
                            </Button>
                        </Right>
                    </ListItem>
...
const mapStateToProps = ({ contactsReducer }) => {
    const { searchName, contactList, filteredContactList, loggedinUser } = contactsReducer;
    return { searchName, contactList, filteredContactList, loggedinUser };
};

export default connect(mapStateToProps, {
    searchNameChanged, updateContactsList
})(Contacts);