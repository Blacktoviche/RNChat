...
async function proccessModal(dispatch, imagePath) {
    if (Platform.OS === 'android') {
        ...
        if (granted === true) {
            ...
            if (imagePath.startsWith(CONSTANTS.ANDROID_FILE_PREFIX + CONSTANTS.IMAGES_DIRECTORY_ANDROID) === true) {
                dispatch({
...
export const startRecordingAudio = () => {
    prepareRecordingPath(RNFetchBlob.fs.dirs.CacheDir + '/' + new Date().getTime() + '.aac');
...
async function prepareRecordingAudio(dispatch) {
    if (Platform.OS === 'android') {
        ...
        if (granted === true) {
            ...
            console.log("Permission denied");
            alert('You cant send/recieve audio unless you grant RNChat app audio record permission');
            dispatch({
                type: EMPTY_ACTION
            });
        }
...
async function requestAudioRecordPermission() {
    var audioRecordPerm = await PermissionsAndroid.check(PermissionsAndroid.PERMISSIONS.RECORD_AUDIO);
    ...
    if (audioRecordPerm) {
        ...
        try {
            granted = await PermissionsAndroid.requestMultiple(
                [PermissionsAndroid.PERMISSIONS.READ_EXTERNAL_STORAGE, PermissionsAndroid.PERMISSIONS.RECORD_AUDIO],
                {
                    'title': 'RNChat App Needs Record Audio Permission',
                    'message': 'RNChat app needs access to your mic ' +
