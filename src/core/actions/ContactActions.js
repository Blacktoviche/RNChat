...
export const updateContactsList = (loggedinUid) => {
    ...
    backend.getLocalContacts().forEach((contact) => {
        contactList.push({ uid: contact.uid, username: contact.username, email: contact.email, photo: contact.photo });
        contactMap = contactMap.set(contact.uid, { uid: contact.uid, username: contact.username, email: contact.email, photo: contact.photo });
    });
    ...
    backend.getOnlineUsers().once('value', function (snapshot) {
        snapshot.forEach(function (userSnapshot) {
            ...
            if (userData.uid != loggedinUid) {
                contactMap = contactMap.set(userData.uid, { uid: userData.uid, username: userData.username, email: userData.email, photo: userData.photo });
            }
        });
