...
export const realm = new Realm({
    path: "RNChat.realm",
    schema: [UserSchema, ContactSchema, MessageSchema, MessageQueueSchema, ChatSchema]
});
...
const updateChat = (message) => {
    let chats = realm.objects('Chat').filtered(' ( uidFrom = $0 && uidTo = $1 ) OR ( uidFrom = $1 && uidTo = $0 ) ', message.uidFrom, message.uidTo);
...
export const getMessages = (uid, uid2) => {
    let messages = realm.objects('Message').filtered(' ( uidFrom = $0 && uidTo = $1 ) OR ( uidFrom = $1 && uidTo = $0 ) ', uid, uid2);
    return messages.sorted('createdAt', true);
}
